...
- Topology visualization: editor transition edges now render directional
  **arrowheads**, and the Results page gains a **transition graph** — a node-graph
  ...
  thickness ∝ probability (complementing the precise heatmap). It **animates with
  the timeline player**: the current Viterbi state is ringed, node fills show the
  posterior `P(state | t)`, and the active transition edge is highlighted as you
  play. A new **Observed data** panel plots the variable curves under the timeline
  (synced cursor), backed by `GET /api/fit/{id}/series`.
