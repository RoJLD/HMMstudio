...
### Added — Phase A.7 user-facing surface (supervised + semi-supervised)

- **`hmm-fit run --labels states.csv`** — single-column CSV of state labels.
  Float `NaN` or integer `-1` sentinel mark unlabelled positions →
  semi-supervised EM; everything labelled → closed-form supervised MLE in one
  pass. 4 new CLI tests cover dispatch, validation (multi-column rejected),
  semi-supervised routing, and `--help` documentation.
- `examples/topology_supervised_3state.yaml` +
  `examples/data_supervised.csv` + `examples/states_supervised.csv` +
  `examples/states_semi_supervised.csv` — canonical 3-state Gaussian dataset
  for the README quickstart and CLI smoke tests. Regenerated by
  `examples/generate_demo_data.py`.
- README **"Supervised & semi-supervised training"** section: 3-mode table
  (unsupervised / supervised / semi-supervised), correct `states=` snippet,
  and a CLI block that maps 1-to-1 to the shipped flag.
- The engine itself was already shipped (`fit(topo, X, states=...)` with 41
  passing tests in `test_supervised.py` + `test_semi_supervised.py`) — this
  release closes the user-facing gap (CLI + examples + correct docs) so A.7 is
  now usable end-to-end without reading the test suite.

### Fixed

- README API drift: the supervised snippet was calling a non-existent
  `state_labels=` kwarg. Now uses the actual `states=` parameter, matching
  `fit()`'s signature and the test suite.
